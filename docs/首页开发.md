...
现在，这里就是一个空空的代码框架，这个框架中代码都是之前介绍基础知识的时候讲过的。
...
然后，显示广告。广告就是显示几个带连接的图片而已，—subpage，先创建 subpage 然后再在 subpage 获取数据，并将数据传递到组件中，最终将广告显示出来。
...
打开`./app/router/routeMap.jsx`可以看到整个webapp的路由配置，目前在配置中。
...
Font Awesome 会一次性全部给你提供好几百个图标，供你选择使用。但是做项目时，根本用不了那么多图标，只用十几个就够了，加载过多会影响性能。
...
这里轮播图，其内容其实是商户的各个类型，而且这些类型比较固定，因此这里不让它涉及过多的数据交互，这些类型都暂时写死到代码中。这样做也是为了更加精确的将当前内容定位到轮播图的讲解上，不让其他关系不大的内容分心太多。因此，就将轮播图做成一个组件。

在`./app/components`下创建`Category`文件夹，存放轮播图组件的代码。

要做轮播图，需要用到`react-swipe`这个插件， https://github.com/voronianski/react-swipe 可以查看文档，根据文档提示需要`npm install swipe-js-iso whatwg-fetch --save`。

这部分的开发分为以下步骤：

- 先不要管类型数据如何，先使用`react-swipe`做一个最简单的轮播图，先让轮播图动起来，并且能控制滚动的间隔时间。
- 然后，再在轮播图下面，加上一个轮播图滚动时 index 标识，即下面的“小圆点儿”，跟着代码看即可
- 轮播图出来之后，再把正式的类型加到轮播图中，并加上样式

## 超值特惠（广告）

上面做的 HomeHeader 和 Category 都是创建在`./app/components`中，即木偶组件，因为不涉及到数据交互。而这里的广告，特意安排它需要后台数据。

在开始做前端代码之前，先看一下后端的接口。终端运行`npm run mock`之后，就会启动后端接口，然后访问`http://localhost:3000/api/homead`就能返回一个JSON格式字符串，这就是广告的数据源。

接下来看`./app/fetch/home/home.js`这个文件，找到获取广告数据的函数。
...
先交代一下后端数据的接口。打开`./app/fetch/home/home.js`，找到获取列表数据的函数
...
接下来在`./app/components/`中创建`LoadMore`组件，并用上上面传递过来的数据。经过上面的准备，该组件将变得非常简单。通过代码一解释，就会非常清楚。
